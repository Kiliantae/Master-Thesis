...
method = ["expected_error_reduction"]
zoom = [100]
...
for med in method:
    for epoch in range(0,5): 
        ...
        for z in zoom:
            ...
            for tss in train_set_size:
                ...
                for train_index, test_index in kf.split(X_split):
                    ...
                    for idx in range(n_queries):
                        query_idx, query_instance = learner.query(X0)
                        learner.teach(
                            X=X0[query_idx].reshape(1, -1),
                            y=y0[query_idx].reshape(1, )#TEACH
                        )
                        #add queried instance to labeled set
                        X_train =  np.concatenate((X_train,X0[query_idx]), axis=0)   
                        y_train =  np.concatenate((y_train,y0[query_idx]), axis=0)   
                        w_train =  np.concatenate((w_train,w0[query_idx]), axis=0)     
            ...
            writer = pd.ExcelWriter("C:/Users/kbock/Desktop/Arbeitsordner/results/{}_MLP_/frac/bacc/{}/{}.xlsx".format(z,med,epoch))
            table.to_excel(writer)
            writer.save()
            writer2 = pd.ExcelWriter("C:/Users/kbock/Desktop/Arbeitsordner/results/{}_MLP_/frac/bacc_cluster/{}/{}.xlsx".format(z,med,epoch))
            table2.to_excel(writer2)
            writer2.save()
            writer3 = pd.ExcelWriter("C:/Users/kbock/Desktop/Arbeitsordner/results/{}_MLP_/frac/f1/{}/{}.xlsx".format(z,med,epoch))
            table3.to_excel(writer3)
            writer3.save()
            writer4 = pd.ExcelWriter("C:/Users/kbock/Desktop/Arbeitsordner/results/{}_MLP_/frac/f1_cluster/{}/{}.xlsx".format(z,med,epoch))
